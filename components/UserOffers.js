...
function SkeletonImage({className = ""}) {
    return <Skeleton className={`aspect-[16/10] w-full ${className}`}/>;
}
...
function OfferCard({item}) {
    ...
    return (<div className="relative">
        ...
        {item.rentierId && (<div
            className="absolute top-2 right-2 bg-black text-white text-xs font-semibold px-2 py-1 rounded-lg z-50">
            Wynajęte
        </div>)}
