                        ...
                        <div className={"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[16px] text-black"}>
                            {
                                offers.map((item, index) => {
                                    if(item.landlordId !== userId) return;

                                    return (
                                        <div key={index} className={"relative"}>
                                            <div
                                                className={"bg-gray-100 px-4 py-1 rounded-lg group absolute z-50 top-2 left-2"}>
                                            ...
                                            <div className={"relative w-full h-[180px] rounded-t-lg overflow-hidden"}>
                                                <Image
                                                    ...
                                                    alt={"app"} layout="fill" className={"object-cover"}/>
                                            </div>
                                            <div className={"bg-white p-[16px] rounded-b-lg shadow-md"}>
                                                <Link href={`/dashboard/rental-offers/${item.id}`}
                                                    ...
                                                    <div className={"flex flex-col items-start gap-1"}>
                                                        <span
                                                            className={"group-hover:underline text-[20px] font-semibold"}>{item.apartment.streetName} {item.apartment.buildingNumber}</span>
                                                        <div>
