...
const GalleryTile = memo(function GalleryTile({img, index, onDelete}) {
    const src = ImageUrl(img.publicUrl);
    const handleDelete = useCallback(() => {
        // placeholder – tylko log
        console.log("click");
        onDelete?.(img, index);
    }, [img, index, onDelete]);
...
const GalleryGrid = memo(function GalleryGrid({images, onDelete}) {
    if (!images?.length) return null;

    return (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
            {images.map((img, i) => (
                <GalleryTile key={img?.id ?? i} img={img} index={i} onDelete={onDelete} />
            ))}
        </div>
    );
});
...
export default function UploadImages({appId}) {
    ...
    const canUpload = useMemo(
        ...
        [appId, accessToken, selectedFiles, isUploading]
    );
    ...
    const refreshImages = useCallback(async () => {
        ...
        try {
            const res = await fetch(`/api/apartments`, {
                ...
                body: JSON.stringify({accessToken, id: appId}),
            });
            const data = await res.json();
    ...
    }, [appId, accessToken, refreshImages]);

    // Placeholder do usuwania — na razie tylko log
    const handleDeleteImage = useCallback((img, index) => {
        // W przyszłości możesz tu zawołać endpoint kasujący:
        // await fetch("/api/apartments/delete-image", { method: "POST", body: JSON.stringify({...}) })
        console.log("click");
    }, []);

    return (
        <>
            {appImages && <Debugger data={appImages} />}
            ...
            {appImages && appImages.length > 0 && (
                <DashboardElement>
                    <div className="flex-col gap-4">
                        ...
                        <GalleryGrid images={appImages} onDelete={handleDeleteImage} />
                    </div>
                </DashboardElement>
            )}
        </>
