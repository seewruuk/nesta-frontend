...
export async function POST(req) {
    try {
        ...
        if (id) {
            ...
            return NextResponse.json({
                status: 200, message: "Apartment fetched successfully", apartment: json
            })
        }
        ...
        return NextResponse.json({
            status: 200, message: "Apartments fetched successfully", apartments: json
        })

    } catch (err) {
        return NextResponse.json({
            status: "error", message: "There was an error fetching the apartments", error: err || "Unknown error"
        });
    }
