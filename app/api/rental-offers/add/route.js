...
export async function POST(req) {
    ...
    try {
        ...
        if (response.status === 409) {
            return NextResponse.json({
                status: 409, message: "Conflict error. The offer might already exist.", error: "Conflict"
            });
        }
        return NextResponse.json({status: "success", message: "Offer saved successfully", data: result}, {status: 200});
    } catch (error) {
